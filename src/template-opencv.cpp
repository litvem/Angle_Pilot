...
// /*-------------------------------------- v ---------------------------------------------------*/
                ...
                cv::Scalar lower_blue = cv::Scalar(82, 100, 23);
                cv::Scalar upper_blue = cv::Scalar(139, 179, 255);
                cv::Scalar lower_yellow = cv::Scalar(20, 100, 120);
                cv::Scalar upper_yellow = cv::Scalar(147, 243, 255);
                ...
                // the merged_mask, this means that only the pixels that are set to white in the merged_mask will be evaluated by the bitwise_and 
                // operation, the rest of the pixels in the result will automatically be set to black.
                cv::Mat result;
                ...
                // cv::GaussianBlur(resultGray, resultGray, cv::Size(3,3), 0);
                // cv::dilate(resultGray, resultGray, 0);
                // cv::erode(resultGray, resultGray, 0);
...
// /*-------------------------------------- v ---------------------------------------------------*/
                ...
                resultGray = resultGray(Range(270, 370), Range(0, 640));
                img = img(Range(270, 370), Range(0, 640));
                ...
                //threshold
                // THRESH_BINARY means that each pixel is compared to the threshold value (127) and if it is larger, then it will be set to 255 == white. 
                ...
                // MORPH_CLOSE specifies that we want to do that kind of operation on the contours. 
                ...
                // Moments are objects that will contain some information about each shape. We can use it to calculate are, orientation size etc.
                ...
                cv::imshow("Triangles", imgContours);
